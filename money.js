...
function renderIncome(){const list=document.getElementById('income-list');if(!list)return;list.innerHTML='';const pay=getSchoolPay();[{source:'Job (school para)',amt:pay,cat:'Primary',color:'#4A6FA5'},...incomeLog.slice(0,10)].forEach(item=>{const e=document.createElement('div');e.style.cssText='display:flex;align-items:center;gap:8px;padding:6px 9px;background:white;border:1px solid var(--cbr);border-radius:8px;font-size:11px;margin-bottom:4px';e.innerHTML=`<div style="width:6px;height:6px;border-radius:50%;background:${item.color||'#4A6FA5'};flex-shrink:0"></div><span style="font-weight:500;flex:1">${item.source}</span><span style="font-size:10px;color:var(--tmut)">${item.cat}</span><span style="font-weight:500;color:var(--green)">$${item.amt}</span>`;list.appendChild(e)});document.getElementById('total-hustle').textContent='$'+incomeLog.reduce((a,b)=>a+b.amt,0)}
...
document.getElementById('inc-save').onclick=()=>{const source=document.getElementById('inc-source').value.trim(),amt=parseFloat(document.getElementById('inc-amt').value)||0;if(!source)return;incomeLog.push({source,amt,cat:document.getElementById('inc-cat').value,color:'#4A6FA5'});ss(K.income,incomeLog);renderIncome();incomeModal.classList.remove('open');document.getElementById('inc-source').value='';document.getElementById('inc-amt').value=''};

renderIncome();renderWishlist();

[{phase:'Now',action:'$1,005/mo job · bumps to $1,500 Aug 12 · survive & build',color:'#5C0300'},{phase:'Soon',action:'UGC brand deals · $50–200/video',color:'#5C0300'},{phase:'Q3 2026',action:'Etsy shop + first doll sales',color:'#4A6FA5'},{phase:'Q4 2026',action:'YouTube monetization threshold',color:'#4A6FA5'},{phase:'2027',action:'Patreon for mods · YouTube ad rev',color:'#5C0300'},{phase:'Goal',action:'$3,000+/mo · beauty fund funded',color:'#930500'}].forEach(r=>{const e=document.createElement('div');e.style.cssText='display:flex;gap:10px;padding:6px 0;border-bottom:1px solid var(--cbr);align-items:center;font-size:11px';e.innerHTML=`<span style="font-size:10px;font-weight:500;color:${r.color};min-width:68px;flex-shrink:0">${r.phase}</span><span>${r.action}</span>`;document.getElementById('money-roadmap').appendChild(e)});
