...
function syncToLifeDashboard(entry){
  try{
    ...
    lifeLog.unshift({source:`${entry.section.toUpperCase()} — ${entry.source}`,amt:entry.amount,cat:entry.section,color:'#e91e8c'});
...
function renderBaddieIdeas(){const c=document.getElementById('baddie-ideas');if(!c)return;c.innerHTML='';baddieIdeas.forEach((idea,i)=>{const platColor={TikTok:'#e91e8c',Instagram:'#e67e22',Both:'#8e44ad','YouTube Short':'#c0392b'}[idea.platform]||'#888';const e=document.createElement('div');e.className='idea-item';e.innerHTML=`<div class="idea-dot"></div><span style="flex:1">${idea.text} <span class="tag" style="background:${platColor}18;color:${platColor}">${idea.platform}</span></span><button class="bdel" onclick="removeBaddieIdea(${i})">×</button>`;c.appendChild(e)})}
...
function renderUGCDeals(){const c=document.getElementById('ugc-deals-list');if(!c)return;c.innerHTML='';if(!ugcDeals.length){c.innerHTML='<div class="empty-s">No UGC deals yet. Start pitching! 💪</div>';return}const statusColor={Pitched:'#888',Confirmed:'#2980b9','In production':'#f0a500',Delivered:'#8e44ad',Paid:'#27ae60'};ugcDeals.forEach((d,i)=>{const sc=statusColor[d.status]||'#888';const e=document.createElement('div');e.className='deal-card';e.innerHTML=`<div style="display:flex;align-items:flex-start;gap:8px"><div style="flex:1"><div class="deal-brand">${d.brand}</div><div class="deal-meta">${d.deliverable||''} ${d.deadline?'· Due: '+d.deadline:''}</div></div><span class="deal-status" style="background:${sc}18;color:${sc};border:1px solid ${sc}33">${d.status}</span><div class="money-amount">${fmtMoney(d.rate)}</div></div>`;c.appendChild(e)})}
...
function renderEmpireMoney(){
  ...
  const colors={youtube:'#c0392b',modding:'#8e44ad',etsy:'#f0a500',dolls:'#e91e8c',baddie:'#e67e22',streaming:'#2980b9'};
...
function renderEmpireRoadmap(){const c=document.getElementById('empire-roadmap');if(!c)return;[{phase:'Now (2026)',action:'School paycheck $1,005/mo · bumps to $1,500 on Aug 12 · Learn the tools',color:'#e67e22'},{phase:'Q2 2026',action:'First Etsy sales + UGC deals · $100–300/mo side hustle',color:'#f0a500'},{phase:'Q3 2026',action:'YouTube monetization threshold · doll sales growing',color:'#27ae60'},{phase:'Q4 2026',action:'Twitch Affiliate · brand deals rolling in · $500–1k/mo hustle',color:'#2980b9'},{phase:'2027',action:'Patreon + mod income + YouTube ads · $1k+ side hustle',color:'#8e44ad'},{phase:'Dream',action:'Full creative living · $3k+/mo · all your jobs are your passion',color:'#e91e8c'}].forEach(r=>{const e=document.createElement('div');e.style.cssText='display:flex;gap:10px;padding:7px 0;border-bottom:1px solid var(--border);align-items:center;font-size:11px';e.innerHTML=`<span style="font-size:10px;font-weight:700;color:${r.color};min-width:90px;flex-shrink:0">${r.phase}</span><span>${r.action}</span>`;c.appendChild(e)})}
